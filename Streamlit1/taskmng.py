...
def init_db():
    ...
    conn.commit()
    conn.close()
    init_sample_data()  # Initialize sample data after creating tables
...
def register_user(username, password, role, name):
    ...
    try:
        # Store role exactly as selected
...
def reassign_deputy_secretary(new_deputy_id):
    ...
    try:
        ...
        c.execute('UPDATE users SET role = "assistant_secretary" WHERE role = "deputy_secretary"')
        # Then assign new deputy
        c.execute('UPDATE users SET role = "deputy_secretary" WHERE id = ?', (new_deputy_id,))
        conn.commit()
        return True
    except Exception as e:
        st.error(f"Error reassigning deputy: {str(e)}")
        return False
    finally:
        conn.close()
...
def add_task(user_id, task_description, status="Pending"):
    conn = sqlite3.connect('task_management.db')
    c = conn.cursor()
    try:
        c.execute('''INSERT INTO tasks (user_id, task_description, status, date)
                    VALUES (?, ?, ?, ?)''',
                 (user_id, task_description, status, datetime.now().date()))
        conn.commit()
        return True
    except Exception as e:
...
def get_user_tasks(user_id):
    conn = sqlite3.connect('task_management.db')
    c = conn.cursor()
    c.execute('''SELECT id, task_description, status, date 
                 FROM tasks 
                 WHERE user_id = ? 
        ...
        with col2:
            ...
            if st.button("Sign Up", key="signup_button"):
                if new_password != confirm_password:
                    ...
                    if register_user(new_username, new_password, new_role, new_name):
    ...
    # Main application after login
        ...
        if st.session_state.user_role == "Deputy Secretary":
            ...
            with tab1:
                ...
                if all_tasks:
                    ...
                    fig_trend = px.line(df_all.groupby(['Date', 'Assistant', 'Status']).size().unstack(fill_value=0),
                                      title="Task Completion Trend by Assistant",
                                      labels={'value': 'Number of Tasks', 'Date': 'Date'})
                    st.plotly_chart(fig_trend, use_container_width=True)
                    
                    # Task Distribution by Assistant
                    fig_dist = px.bar(df_all.groupby(['Assistant', 'Status']).size().unstack(fill_value=0),
                                    title="Task Distribution by Assistant",
                                    labels={'value': 'Number of Tasks'})
            ...
            with tab4:
                ...
                if assistants:
                    ...
                    if st.button("Reassign Deputy Secretary"):
                        ...
                        if reassign_deputy_secretary(new_deputy_id):
                            st.success("Deputy Secretary reassigned successfully!")
                            st.rerun()
                else:
                    st.info("No assistant secretaries found")
        ...
        # Assistant Secretary View
            ...
            if tasks:
                ...
                df = pd.DataFrame(tasks, columns=['ID', 'Task', 'Status', 'Date'])
                ...
                st.dataframe(df[['Task', 'Status', 'Date']])
            else:
